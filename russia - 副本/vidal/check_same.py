...
def connection():
    ...
    cli = pymongo.MongoClient('192.168.16.158',connect = False)
    return cli
...
#     cli = connection()
#
#     coll = get_collection(cli, "russia", "info")
#
...
class info_model(object):
    ...
    def duplicate_removal(self, execute=True):
        ...
        res = self.collection.aggregate(agg,**options)
        if execute:
            for i in res:
                i['dups'].pop()
...
if __name__ == "__main__":
    cli = connection()
    coll = get_collection(cli, "russia_db", "info")
    # info_map = {"brand_name": "Brand Name", "dosage_form": "Dosage Form", "company_name": "Company Name",
    #             "packing": "Packaging", "molecule": "Molecule", "category": "Category", "strength": "Strength",
    #             "mrp_rs": "MRP(Rs)"}
