...
class MongoPipeline(object):
    ...
    def process_item(self,item,spider):
        ...
        info_map = {"licensee": "Владелец регистрационного удостоверения", "producer": "Произведено", "atx": "Код ATX",
                    ...
                    "storage": "Условия хранения", "storage_time": "Срок годности", "maker": "Упаковано" , "infoPage": "Контакты для обращений"}
        save_map = {}
        postItem = dict(item)
        for k, v in info_map.items():
            save_map[v] = postItem[k]
        self.db[self.mongo_coll].insert(dict(save_map))
        return item
...
class HtmlPipeline(object):
    ...
    def from_crawler(cls,crawler):
        return cls(
            mongo_uri = crawler.settings.get('MONGO_URI'),
            mongo_db = crawler.settings.get('MONGO_DB'),
            mongo_coll=crawler.settings.get('HTML_COLL')
        )
